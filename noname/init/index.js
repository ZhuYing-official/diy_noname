...
export async function boot() {
	...
	if (!localStorage.getItem(lib.configprefix + 'disable_extension')) {
		...
		for (var name = 0; name < config.get('extensions').length; name++) {
			...
			else if (get.config('extensions_enable', 'connect') || config.get('mode') != 'connect' || (!localStorage.getItem(lib.configprefix + 'directstart') && show_splash)) {
		...
		if (get.config('extensions_enable', 'connect') || config.get('mode') != 'connect' || (!localStorage.getItem(lib.configprefix + 'directstart') && show_splash)) {
			var alerted = false;
			for (var name = 0; name < config.get('extensions').length; name++) {
				if (Reflect.get(window, 'bannedExtensions').includes(config.get('extensions')[name])) {
					//if(!alerted) alert('读取某些扩展时出现问题。');
	...
	if (extensionlist.length && (config.get('mode') != 'connect' || show_splash || get.config('extensions_enable', 'connect'))) {
