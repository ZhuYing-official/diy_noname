...
const skills = {
		...
		async content() {
			try {
				let zhuskill = function () {
					...
					// 1/3概率选择tianming，2/3概率从其他8个技能中随机选择
					if (Math.random() < 1 / 3) {
						console.log('选择了天命');
						return 'tianming';
					} else {
						let laoRandom = Math.floor(Math.random() * 8);
						let selectedSkill = laoSkills[laoRandom];
						console.log('选择了其他技能:', selectedSkill);
						return selectedSkill;
					}
				}

				let selectedSkill = zhuskill();
				// 确保storage存在
				if (!game.zhu.storage) game.zhu.storage = {};
				game.zhu.storage.enhance_zhu = selectedSkill;
				// 添加技能
				player.addSkill(selectedSkill);
			} catch (e) {
				console.log('Enhance_zhu 错误:', e.message);
				console.error('Enhance_zhu 错误:', e);
			}
		},
	},
	_Enhance_zhu_remove: {
	...
	lao_badou2: {
		...
		filter(event, player) {
			...
			if (filter(get.autoViewAs({ name: 'jiu' }, 'unsure'), player, event) && player.countCards('hs', { suit: 'club' })) {
				return true;
			}
			return false;
		},
