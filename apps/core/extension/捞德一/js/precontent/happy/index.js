...
const packs = function () {
	...
	for (var i in happy.character) {
		if (Array.isArray(happy.character[i])) happy.character[i] = get.convertedCharacter(happy.character[i]);
		if (_status['extension_捞德一_files']) {
			const files = _status['extension_捞德一_files'];
			if (files.image.character.files.includes(`${i}.jpg`)) happy.character[i].img = `extension/捞德一/image/character/${i}.jpg`;
		}
